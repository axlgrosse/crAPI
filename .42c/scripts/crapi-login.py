...
# Obtain a JWT token using login to crAPI app. 
...
def obtain_token(name: str, password: str, target_url: str, quiet: bool, debug: bool):
    ...
    retry_count = 0
    url = f"{target_url}/identity/api/auth/login"
    headers = {"accept": "application/json", "Content-Type": "application/json"}

    payload = {f"email": name, "password": password}
...
def main():
    parser = argparse.ArgumentParser(
        description='crAPI API Login'
    )
    parser.add_argument('-u', "--user-name",
                        default="UserName",
                        help="crapiApp User", required=True)
    parser.add_argument('-p', "--user-pass",
                        help="crapiApp Password", required=True)
